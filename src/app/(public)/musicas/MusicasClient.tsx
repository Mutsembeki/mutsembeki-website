...
export function MusicasClient({ songs, categories, albums }: MusicasClientProps) {
  ...
  const handleDownload = async (songId: string, songTitle: string) => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = url
        a.download = `mutsembeki-${songTitle.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-')}.mp3`
        a.click()
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedCategory === 'all' ? 'bg-gold text-background' : 'glass text-muted-foreground hover:text-gold'
                  }`}
              >
                Todas
              </button>
              {categories.map(cat => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedCategory === cat.id ? 'bg-gold text-background' : 'glass text-muted-foreground hover:text-gold'
                    }`}
                >
                  {cat.name}
                </button>
